...
uint16_t ccenterx,ccentery;//center x,y of the clock
  ...
  uint16_t cradius = 63;//radius of the clock
  float scosConst = 0.0174532925;
...
void drawClockFace()
{
    ...
    uint8_t i;
  drawFillCircle(_GRAMWIDTH / 2, _GRAMHEIGH / 2, cradius, BLUE);
  drawFillCircle(_GRAMWIDTH / 2, _GRAMHEIGH / 2, cradius-4, BLACK);
  // Draw 12 lines
...
void setup_clock(void) 
{
  ...
  ccenterx = _GRAMWIDTH/2;
  ccentery = _GRAMHEIGH/2;
  osx = ccenterx;
  osy = ccentery;
  omx = ccenterx;
  omy = ccentery;
...
void drawClockHands(uint8_t h,uint8_t m,uint8_t s)
{
  ...
  // Erase just old hand positions
   drawLine(ohx, ohy, ccenterx+1, ccentery+1, BLACK);  
   drawLine(omx, omy, ccenterx+1, ccentery+1, BLACK);  
   drawLine(osx, osy, ccenterx+1, ccentery+1, BLACK);
  // Draw new hand positions  
   drawLine(hx*(cradius-28)+ccenterx+1, hy*(cradius-28)+ccentery+1, ccenterx+1, ccentery+1, WHITE);
  drawLine(mx*(cradius-17)+ccenterx+1, my*(cradius-17)+ccentery+1, ccenterx+1, ccentery+1, WHITE);
   drawLine(sx*(cradius-14)+ccenterx+1, sy*(cradius-14)+ccentery+1, ccenterx+1, ccentery+1, RED);
   drawFillCircle(ccenterx+1, ccentery+1, 3, RED);
